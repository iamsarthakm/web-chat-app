...
module.exports.sockets = function(http) {
  ...
  eventEmitter.on("save-chat",async function(data) {
    // var today = Date.now();
    const urlAdd='http://127.0.0.1:8080/predict_text?api_key=admin123admin&userid=1';
    const res=await request({url: urlAdd,
      method: "POST",
      json: {'message':data.msg}
      },async function (error, response, body) {
      var globalpred= await body.pred;
      
      if(globalpred===1){
        console.log('emit')
        //data.socket('distracting-found',{'dat':1});
        data.socket.emit('distracting', {
          id: data.date
        });
      }
