$ (function(){
  ...
  socket.on('onlineStack',function(stack){
    $('#list').empty();
    $('#list').append($('<li>').append($('<button id="ubtn" class="btn btn-info btn-block btn-lg"></button>').text("Group").css({"font-size":"18px"})));
    var totalOnline = 0;
    for (var user in stack){
      //setting txt1. shows users button.
      if(user == username){
        var txt1 = $('<button class="btn btn-light  btn-md disabled"> </button>').text(user).css({"font-size":"18px"});
      }
      else{
        var txt1 = $('<button id="ubtn" class="btn btn-info  btn-md">').text(user).css({"font-size":"18px"});
      }
    ...
    if(data.room == roomId){
      ...
      if(data.result.length != 0){
        ...
        for (var i = 0;i < data.result.length;i++) {
          ...
          if (data.result[i].distracting==1){
               $('#messages').prepend($(`<li id=${data.result[i].createdOn} style="background:lightcoral;opacity:.3">`).append(txt3,txt4));
              }else
             $('#messages').prepend($(`<li id=${data.result[i].createdOn}>`).append(txt3,txt4));
          msgCount++;
